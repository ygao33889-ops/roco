...
body{max-width:1100px;margin:0 auto;padding:20px;background:#f0f2f5;}
...
<div class="card">
    ...
    <table>
        <thead><tr><th>名字</th><th>性别</th><th>蛋组</th><th>加成</th><th id="petOpHead" class="hidden">操作</th></tr></thead>
...
async function loadPets(){
    ...
    arr.forEach((item,idx)=>{
        ...
        tr.innerHTML = `<td>${item.name}</td><td>${item.sex}</td><td>${item.egg_group}</td><td>${item.bonus}</td>${op}`;
        tb.appendChild(tr);
    });
}
async function addPet(){
    ...
    let r = await api("/api/pets/add","POST",{name,sex,egg_group:eg,bonus});
    if(r.ok){ ["petName","petEggGroup","petBonus"].forEach(id=>document.getElementById(id).value=""); loadPets(); }
}
...
window.onload = ()=>{ checkAuth(); loadPets(); loadEggs(); };
</script>
</body>
</html>
'''
...
def pet_add():
    ...
    j = request.get_json()
    d = load_data()
    d["pets"].append({"name": j["name"], "sex": j["sex"], "egg_group": j["egg_group"], "bonus": j["bonus"]})
    save_data(d)
    return jsonify({"ok": True})
